...
def mfa_atm(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if (perfil_mfa.mfa_habilitado_transacciones and
...
def procesar_billetes_venta(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        try:
            id_tauser = Transaccion.objects.filter(id_transaccion=transaccion_id).values_list('tauser__id', flat=True).first()
